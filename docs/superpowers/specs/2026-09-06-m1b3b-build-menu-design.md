...
- **選單與建塔點美術重疊。** 兩個選項時，兩側按鈕各與 `prop_buildsite` 重疊約 69×32px；塔升到三階（`tower_musket_t3` 是 192×192）時，升級與賣出按鈕會蓋住約 101×64px——正好是玩家剛花 230 想看看買到什麼的時候。另外 `custom_minimum_size` 是下限不是上限，英文語系的 `Upgrade 130` 可能把按鈕撐得比 110 寬，而相鄰按鈕的中心只差 100。**已列入人工驗收項目，依驗收結果再決定要不要調半徑或按鈕尺寸。**
- **三個選項時重疊更嚴重**，但本關最多兩個選項（兩種塔；升級＋賣出也是兩個），加第三種塔時要一併重看。
- **`_current_menu_structural_signature()` 每個渲染幀仍配置一個三元素 Array。** 修正 #1 的重構沒有消掉它（實作者的報告誤記為已消失）。這是 `_process` 不是 30Hz 的 tick，`CLAUDE.md` 規則 5 針對的是後者，故不處理。
- **`ui/` 分層守衛的寫入偵測只看得到有明確型別註記的名稱。** 目前 `build_menu_options.gd` 剛好每個持有的參照都有註記，所以涵蓋得到；若日後某個 `ui/` 檔案全用 `:=` 推斷，該檔的寫入偵測會靜默失效，而套件層級的「至少看到一個持有參照」斷言不會發現——因為 `battle_hud.gd` 一個檔案就滿足它。
- **升級與賣出按鈕沒有 tooltip**，只有建造有（建造需要 tooltip 是因為圖示本身給不出塔名）。等長按查看詳細數值時一併處理。
- **塔的 `flip_h` 在目標死亡後會停在最後一次的朝向**，直到鎖定新目標。與舊的 rotation 行為相同，且視覺上「維持面向最後開火的方向」本來就比彈回預設好。真正的解法屬於待機／開火動畫。
- **`_find_tower_view_owner` 已經是「依 id 找塔」線性搜尋的第三份**（另兩份在 `BuildSystem` 與 `BuildMenuOptions`）。三份都是刻意的——分層守衛不允許 `ui/` 認識 `BuildSystem`——但名字該改，它早就不只服務 view 的清理。
- **選單佔用建塔點下方約 210×155 的矩形並吃掉其中的點擊。** 目前四個建塔點間距足夠，但**下一關的建塔點垂直間距要保持 160 以上**，否則選中一個會讓鄰近的點不可點，而且沒有任何測試抓得到。
- **選單的定位假設「場景根在原點且沒有 Camera2D」**，把世界座標直接餵進 `CanvasLayer`。已在呼叫點加註解；加入鏡頭的那天要一併處理。
