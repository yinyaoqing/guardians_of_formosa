...
**唯一的例外是暫停中**：`advance()` 在 `paused` 為真時仍會排空佇列，之後才提前返回，其餘 tick 步驟一概不跑。理由是建造與賣出正是玩家暫停下來規劃時該做的事；若佇列在暫停時只進不出，恢復的那一刻會一次套用整個積壓的佇列。代價是這些變更發生在任何 tick 之外，因此不帶 tick 編號——見 §2.2 的但書。

### 2.2 理由

輸入發生在渲染幀上，模擬跑 30Hz 固定步長，兩者不對齊。排入佇列讓**所有改變世界的事情都發生在 tick 內的明確位置**——這正是既有整套 tick 順序測試賴以成立的前提。副產品「灌一串 intent 即可完整重現一場戰鬥」目前尚**不**成立，見下方的但書。

代價是最多一個 tick（約 33 毫秒）的延遲，肉眼不可察覺；以及表現層不得假設「按下去塔就存在」，需等下一次同步。

**但書（2026-09-04，M1-B1 review 補記）：** 上一段「灌一串 intent 即可完整重現一場戰鬥」的說法在本分支完成時尚未成立。`game/level/battle_scene.gd` 目前仍在 `_process()`（渲染幀）中依渲染 delta 產生敵人波次，敵人的生成時間點因此隨幀率浮動、不受 `paused` 影響、也不隨 `speed_multiplier` 縮放。intent 佇列只涵蓋了「玩家操作」這一路輸入，戰鬥的另一路輸入（敵人波次）仍在 tick 之外，兩路合起來才是完整的戰鬥輸入，回放與離線平衡模擬要成立，兩路都必須是確定性的。這個成立條件依附於波次系統：**敵人生成何時搬進 tick 內、由 `pending_intents` 或等價的排隊機制驅動，屬於波次系統子里程碑（M1 之後）的範圍**，不在本分支內處理。

**第二個例外（同日補記）：** §2.1 的暫停排空同樣落在 tick 之外。它的影響範圍比敵人生成小得多——暫停時世界只會因玩家的建造與賣出而改變，而那些變更本身仍是確定性的、順序仍由佇列決定——但嚴格來說「所有改變世界的事情都發生在 tick 內」在暫停期間並不成立。回放機制屆時需要為暫停期間的指令記錄一個位置（例如「第 N 個 tick 之前」），而不是假設每筆指令都落在某個 tick 內。

另有一個表現層的連帶影響待 B3 處理：`battle_scene.gd` 只在 `ticks > 0` 時同步 view，所以暫停中建造或賣出的塔，畫面要等到恢復才會更新。今天無從觸發（沒有任何程式碼會設 `paused`），但 B3 接上暫停 UI 的那一刻就會浮現。
